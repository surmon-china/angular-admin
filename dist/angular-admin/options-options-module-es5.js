...
var OptionsComponent = /** @class */ (function () {
    function OptionsComponent(router, fb, httpService) {
        var _a;
        this.router = router;
        this.fb = fb;
        ...
        this.fetching = (_a = {},
            _a[ELoading.Auth] = false,
            _a[ELoading.Option] = false,
            _a[ELoading.MusicCache] = false,
            _a[ELoading.BilibiliCache] = false,
            _a[ELoading.GithubCache] = false,
            _a[ELoading.SitemapCache] = false,
            _a);
        // authForm
    ...
    // 长数据处理器
    OptionsComponent.prototype.formatLongString = function (value) {
        return value.replace(/\s+/g, ' ').replace(/\s/g, '\n');
    };
    // ping 地址解析处理
    OptionsComponent.prototype.handlePingSitesChange = function (event) {
        this.seo_ping_sites.setValue(this.formatLongString(event.target.value));
    };
    // 黑名单 ip 解析处理
    OptionsComponent.prototype.handleCommentBlacklistIpsChange = function (event) {
        this.blacklist_ips.setValue(this.formatLongString(event.target.value));
    };
    // 黑名单邮箱解析处理
    OptionsComponent.prototype.handleCommentBlacklistMailsChange = function (event) {
        this.blacklist_mails.setValue(this.formatLongString(event.target.value));
    };
    // 黑名单关键词解析处理
    OptionsComponent.prototype.handleCommentBlacklistKeywordsChange = function (event) {
        this.blacklist_keywords.setValue(this.formatLongString(event.target.value));
    };
    // 关键词计息处理
    OptionsComponent.prototype.handleKeywordsChange = function (event) {
        var newWords = event.target.value.replace(/\s/g, '').split(',');
        this.keywords.setValue(newWords);
    };
    // 提交权限表单
    OptionsComponent.prototype.submitAuthForm = function () {
        if (!this.authForm.valid) {
            return false;
        }
        var authFormData = lodash__WEBPACK_IMPORTED_MODULE_1__["cloneDeep"](this.authForm.value);
        Object.keys(authFormData).forEach(function (key) {
            var value = authFormData[key];
            var isPassword = key.includes('password');
    ...
    // 提交设置表单
    OptionsComponent.prototype.submitOptionForm = function () {
        if (!this.optionForm.valid) {
            return false;
        }
        var format = function (value) { return String(value).split('\n').filter(function (t) { return !!t; }); };
        var formValue = Object.assign({
    ...
    // 解析返回的权限表单数据
    OptionsComponent.prototype.handleAuthChange = function (userAuthPromise) {
        var _this = this;
        userAuthPromise.then(function (_a) {
            var _b = _a.result, name = _b.name, slogan = _b.slogan, gravatar = _b.gravatar;
            if (_this.authForm.value.rel_new_password) {
                // tslint:disable-next-line:no-console
                console.info('密码更新成功，正跳转至登陆页');
                setTimeout(function () { return _this.router.navigate(['/auth']); }, 960);
            }
            else {
                _this.authForm.reset(Object.assign({}, DEFAULT_AUTH_FORM, { name: name, slogan: slogan, gravatar: gravatar }));
            }
        }).catch(function (error) { });
    };
    // 解析返回的设置表单数据
    OptionsComponent.prototype.handleOptionChange = function (optionPromise) {
        var _this = this;
        return optionPromise.then(function (_a) {
            var options = _a.result;
            var format = function (value) { return value.toString().replace(/,/g, '\n'); };
    ...
    // 获取用户
    OptionsComponent.prototype.getUserAuth = function () {
    ...
    // 更新用户
    OptionsComponent.prototype.putAuth = function (auth) {
    ...
    // 更新配置
    OptionsComponent.prototype.putOptions = function (options) {
    ...
    // 更新音乐缓存
    OptionsComponent.prototype.updateMusicCache = function () {
    ...
    // 更新 Bilibili 缓存
    OptionsComponent.prototype.updateBilibiliCache = function () {
    ...
    // 更新 Github 缓存
    OptionsComponent.prototype.updateGithubCache = function () {
    ...
    // 更新网站地图缓存
    OptionsComponent.prototype.updateSitemapCache = function () {
